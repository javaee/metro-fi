...
public class BooleanEncodingAlgorithm extends BuiltInEncodingAlgorithm {
    ...
    public final Object decodeFromInputStream(InputStream s) throws IOException {
        final Vector booleanList = new Vector();
    ...
    public void encodeToOutputStream(Object data, OutputStream s) throws IOException {
        if (!(data instanceof boolean[])) {
            throw new IllegalArgumentException(MessageCenter.getString("message.dataNotBoolean"));
        }
            ...
            new WordListener() {
                public void word(int start, int end) {
                    if (cb.charAt(start) == 't') {
                        booleanList.addElement(Boolean.TRUE);
                    } else {
                        booleanList.addElement(Boolean.FALSE);
                    }
                }
            }
        );
    ...
    public int getPrimtiveLengthFromOctetLength(int octetLength, int firstOctet) throws EncodingAlgorithmException {
        ...
        if (octetLength == 1) {
           if (unusedBits > 3) {
               throw new EncodingAlgorithmException(MessageCenter.getString("message.unusedBits4"));
           }
           return 4 - unusedBits;
        } else {
           if (unusedBits > 7) {
               throw new EncodingAlgorithmException(MessageCenter.getString("message.unusedBits8"));
           }
     ...
     *
     */
    private final boolean[] generateArrayFromList(Vector array) {
        boolean[] bdata = new boolean[array.size()];
        for (int i = 0; i < bdata.length; i++) {
            bdata[i] = ((Boolean)array.elementAt(i)).booleanValue();
        }
